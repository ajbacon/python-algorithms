...
def is_first_come_first_served(take_out_orders, dine_in_orders, served_orders):
    ...
    order_count = len(served_orders)
    counter = 0
    take_out_count = 0
    dine_in_count = 0

    while counter < order_count - 1:
        if served_orders[counter] == take_out_orders[take_out_count]:
            take_out_count += 1
        elif served_orders[counter] == dine_in_orders[dine_in_count]:
            dine_in_count += 1
        else:
            return False
    ...
    print(
        f"out count: {take_out_count}, in count: {dine_in_count}, total {order_count}")
    return True
